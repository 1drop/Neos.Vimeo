...
class VimeoViewHelper extends AbstractViewHelper {
	...
	public function render(array $elements, $thumbnailSize = 2, $showFilter = TRUE, $videosPerRowExtendedDesktop = 'seven', $videosPerRowDesktop = 'five', $videosPerRowTablet = 'three', $videosPerRowMobile = 'one', $defaultStartFilter = '*', $addAllFilter = TRUE ) {
		...
		$output .= '<div class="vimeo-container">';
	...
	public function createHtmlElement($data,$element,$videosPerRowMobile,$videosPerRowTablet,$videosPerRowDesktop,$videosPerRowExtendedDesktop){
		...
		$html .= '<div class="item col-xs-'.$videosPerRowMobile.' col-sm-'.$videosPerRowTablet.' col-md-'.$videosPerRowDesktop.' col-lg-'.$videosPerRowExtendedDesktop.' '.$this->umlaute->convertAccentAndBlankspace($element['name']).'">';
			$html .= '<a href="'.$data['link'].'" target="_self" class="fancybox-media">';
				$html .= '<img src="'.$data['pictures']['sizes'][$this->vimeoThumbnailSize]['link'].'" width="'.$data['pictures']['sizes'][$this->vimeoThumbnailSize]['width'].'" height="'.$data['pictures']['sizes'][$this->vimeoThumbnailSize]['height'].'" alt="'.$data['name'].'" />';
				$html .= '<div class="item-overlay"><span class="title">'.$data['name'].'</span> <span class="description">'.$data['description'].'</span></div>';
			$html .= '</a>';
		$html .= '</div>';
		...
		if($defaultStartFilter == '' || $defaultStartFilter == '*'){
			...
			if(count($elementNames) > 0){
				foreach($elementNames as $filter){
					$html .= '	<button data-filter=".'.$this->umlaute->convertAccentAndBlankspace($filter['name']).'">'.$filter['name'].'</button>';
				}
			}
